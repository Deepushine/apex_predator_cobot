...
def test_pin(ser, pin_name, angle=45):
    ...
    print(f"  - Motor 1 (connected to X-axis/Base)")
    print(f"  - Motor 2 (connected to Y-axis/Shoulder)")
    print(f"  - Motor 3 (connected to Z-axis/Elbow)")
    print()
    
    # FIX: Convert pin name to proper <MOVE> format firmware expects
    if pin_name == "J13":
        command = f"<MOVE X{angle} Y0 Z0>"
    elif pin_name == "J15":
        command = f"<MOVE X0 Y{angle} Z0>"
    elif pin_name == "J17":
        command = f"<MOVE X0 Y0 Z{angle}>"
    else:
        command = f"<MOVE X{angle} Y{angle} Z{angle}>"
    ser.write(f"{command}\n".encode('utf-8'))
...
def main():
    ...
    try:
        ...
        test_pin(ser, "J13", 45)
        time.sleep(1)
        # FIX: Use proper <MOVE> format for return-to-zero instead of invalid J13:0
        ser.write(b"<MOVE X0 Y0 Z0>\n")
        time.sleep(0.5)
        ...
        test_pin(ser, "J15", 45)
        time.sleep(1)
        # FIX: Use proper <MOVE> format for return-to-zero instead of invalid J15:0
        ser.write(b"<MOVE X0 Y0 Z0>\n")
        time.sleep(0.5)
        ...
        test_pin(ser, "J17", 45)
        time.sleep(1)
        # FIX: Use proper <MOVE> format for return-to-zero instead of invalid J17:0
        ser.write(b"<MOVE X0 Y0 Z0>\n")
        time.sleep(0.5)
